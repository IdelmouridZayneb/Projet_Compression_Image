...
# Utilisée par : Agent 5 Rapporteur
...
def interpreter_metrique(psnr_v, ssim_v, mse_v, taux):
    return {
        ...
        "gain": "Excellent" if taux > 50 else "Bon" if taux > 30 else "Faible"
    }
...
# Utilisée par : Agent 5 Rapporteur — calcule PSNR, SSIM, MSE, taux, ratio
...
# ============================================================
# AGENT 2 : DÉCIDEUR — Kaoutar (agent_decideur.py)
# ============================================================

GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
GROQ_URL     = "https://api.groq.com/openai/v1/chat/completions"

SYSTEM_PROMPT = """Tu es un expert en compression d'images.
On te donne les caractéristiques techniques d'une image (JSON).
Tu dois choisir le MEILLEUR format de compression parmi : JPEG, PNG, WebP, AVIF, HEIF.
IMPORTANT : La valeur "qualite" doit TOUJOURS être entre 70 et 95. Jamais en dessous de 70.
Réponds UNIQUEMENT avec un JSON valide, sans texte autour.
Format de réponse :
{
  "format_choisi": "WebP",
  "qualite": 80,
  "raison": "Photo naturelle avec dégradés, WebP offre le meilleur ratio qualité/taille",
  "cas_usage": "web"
}"""

def agent_decideur(features_json: dict) -> dict:
    """Appelle le LLM Groq pour décider du format de compression."""
    user_message = f"""Voici les caractéristiques de l'image à compresser :
{json.dumps(features_json, indent=2, ensure_ascii=False)}
Choisis le meilleur format de compression."""
    payload = {
    ...
    response = req.post(GROQ_URL, json=payload, headers=headers, timeout=30)
...
@app.route('/agent/decideur', methods=['POST'])
def route_decideur():
    data     = request.json
    features = data.get("features")
    if not features:
        return jsonify({"erreur": "Champ 'features' manquant"}), 400
    try:
        resultat = agent_decideur(features)
...
# ============================================================
# AGENT 3 : COMPARATEUR — Kaoutar (agent_comparateur.py)
# ============================================================
...
SYSTEM_PROMPT_COMPARATEUR = """Tu es un expert en compression d'images.
...
def appeler_llm(modele: str, features_json: dict) -> dict:
    """Appelle un LLM Groq spécifique."""
    payload = {
        "model": modele,
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT_COMPARATEUR},
            {"role": "user",   "content": f"Analyse ces caractéristiques et choisis le meilleur format :\n{json.dumps(features_json, ensure_ascii=False)}"}
        ],
        "temperature": 0.1,
        "max_tokens": 250
    }
...
def comparer_et_selectionner(features_json: dict) -> dict:
    """Appelle 2 LLM, compare leurs réponses, retourne la meilleure décision."""
    resultats = {}
    for nom, modele in MODELES.items():
        try:
            print(f"Appel LLM : {nom} ({modele})...")
            resultats[nom] = appeler_llm(modele, features_json)
            resultats[nom]["modele_utilise"] = nom
        except Exception as e:
            print(f"Erreur avec {nom}: {e}")
            resultats[nom] = {"erreur": str(e)}
    decision_finale = selectionner_meilleure_decision(resultats, features_json)
    return {
        "decision_finale": decision_finale,
        "comparaison": {
            "llama_70b": resultats.get("llama_70b"),
            "llama_8b" : resultats.get("llama_8b"),
            "accord"   : resultats.get("llama_70b", {}).get("format_choisi") ==
                         resultats.get("llama_8b",  {}).get("format_choisi")
        }
    }


def selectionner_meilleure_decision(resultats: dict, features: dict) -> dict:
    """Règles de sélection entre les deux LLM."""
    llama   = resultats.get("llama_70b", {})
    mixtral = resultats.get("llama_8b",  {})
    if llama.get("format_choisi") == mixtral.get("format_choisi"):
        decision = llama.copy()
        decision["methode_selection"] = "accord_unanime"
        return decision
    conf_llama   = llama.get("score_confiance",   0.5)
    conf_mixtral = mixtral.get("score_confiance", 0.5)
    if conf_llama >= conf_mixtral:
        decision = llama.copy()
        decision["methode_selection"] = "score_confiance_superieur_llama"
    else:
        decision = mixtral.copy()
        decision["methode_selection"] = "score_confiance_superieur_mixtral"
    type_image     = features.get("type_image", "")
    contient_texte = features.get("ocr", {}).get("contient_texte", False)
    if contient_texte:
        decision["format_choisi"]     = "PNG"
        decision["methode_selection"] = "regle_metier_texte_detecte"
    elif type_image == "screenshot":
        decision["format_choisi"]     = "WebP"
        decision["methode_selection"] = "regle_metier_screenshot"
    return decision


@app.route('/agent/comparateur', methods=['POST'])
def agent_comparateur():
    data     = request.json
    features = data.get("features")
    if not features:
        return jsonify({"erreur": "Champ 'features' manquant"}), 400
    try:
        resultat = comparer_et_selectionner(features)
        return jsonify(resultat)
    except Exception as e:
        return jsonify({"erreur": str(e)}), 500
...
# ============================================================
# AGENT 4 : EXÉCUTEUR — Zayneb (agent_executeur.py)
# ============================================================
# Compresse l'image et sauvegarde dans output_images/
# Supporte : JPEG, PNG, WEBP, AVIF, HEIF

def agent_executeur(chemin_source, format_cible, qualite=70):
    """
    Fonction principale qui réalise la compression physique de l'image.
    """
    try:
        ...
        # 3. Préparer le dossier et le nom de sortie
        dossier_sortie = "output_images"
        if not os.path.exists(dossier_sortie):
            os.makedirs(dossier_sortie)

        nom_base     = os.path.splitext(os.path.basename(chemin_source))[0]
        ext          = format_cible.lower()
        chemin_final = f"{dossier_sortie}/{nom_base}_compresse.{ext}"

        # 4. Logique de compression par format
        fmt_upper = format_cible.upper()

        if fmt_upper == "JPEG":
            img.convert("RGB").save(chemin_final, "JPEG", quality=qualite, optimize=True)
        elif fmt_upper == "PNG":
            # PNG est sans perte (lossless), on utilise l'optimisation
...
def route_executeur():
    """
    Route Flask qui appelle la fonction de Zayneb.
    Retourne l'image compressée + originale en base64 pour le Rapporteur.
    """
    try:
        data = request.json
        if not data:
        ...
        # Lire les images pour les retourner en base64 au Rapporteur
...
# ============================================================
# Fonctions utilisées par cet agent :
#   - calculate_all_metrics() → calcule PSNR, SSIM, MSE, taux, ratio Q/T
#   - evaluate_quality()      → évalue "Excellent/Bon/Acceptable/Dégradé"
#   - interpreter_metrique()  → interprétation textuelle des métriques
#   - matplotlib.pyplot       → génère metrics_chart.png (SSIM + taux)
# Données reçues :
#   - features           ← Agent 1 Analyste
#   - llm_decision       ← Agent 3 Comparateur Kaoutar
#   - compressed_b64     ← Agent 4 Exécuteur (image compressée)
#   - original_b64       ← Agent 4 Exécuteur (image originale)
#   - format_used        ← Agent 4 Exécuteur
#   - quality_used       ← Agent 4 Exécuteur

from pathlib import Path

BASSMA_RAPPORTS_DIR = Path(r"C:\Users\Lenovo ThinkPad T470\OneDrive\Documents\Bureau\Projet_Compression\rapports")
...
def agent_rapporteur():
    try:
        ...
        # Évaluation qualité
        if bassma_ssim_val >= 0.95:   bassma_evaluation = "Excellent"
        elif bassma_ssim_val >= 0.90: bassma_evaluation = "Bon"
        elif bassma_ssim_val >= 0.80: bassma_evaluation = "Acceptable"
        else:                         bassma_evaluation = "Dégradé"
        ...
        # ── Graphique : SSIM + Taux de compression ────────────────────────
        ...
        bassma_ax1.set_title("SSIM", color="white", fontsize=9)
        ...
        bassma_rapport = {
            "rapport_compression": {
                ...
                "conclusion": (
                    f"Compression {bassma_evaluation} — {bassma_tau:.1f}% économisé. "
                    f"PSNR={bassma_psnr_val:.2f} dB | SSIM={bassma_ssim_val:.4f} | MSE={bassma_mse_val:.2f}. "
        ...
        logger.info(f"[Rapporteur — Bassma] {bassma_evaluation} τ={bassma_tau:.1f}% SSIM={bassma_ssim_val:.4f}")
...
def health():
    return jsonify({
        ...
        "llm_provider"     : "Groq (llama-3.3-70b-versatile + llama-3.1-8b-instant)",
...
def run_all_images():
    ...
    dossier = data.get("dossier", r"C:\Users\Lenovo ThinkPad T470\OneDrive\Documents\Bureau\Projet_Compression\database-projet")
...
def list_images():
    dossier = r"C:\Users\Lenovo ThinkPad T470\OneDrive\Documents\Bureau\Projet_Compression\database-projet"
...
def _run_pipeline_thread(dossier):
    """
    Pipeline complet :
    Analyste → Décideur (Groq) → Comparateur (Groq 2 LLM) → Exécuteur (Zayneb) → Rapporteur (Bassma)
    """
    import time
    ...
    for img_path in images:
        ...
        try:
            ...
            # ── Étape 5 : Rapporteur — Bassma (calcule les métriques) ─────
            ...
            path_r = os.path.join(RAPPORTS_DIR, f"{nom}_rapport.json")
...
if __name__ == '__main__':
    ...
    print(f"  AVIF     : ✅")
    print(f"  JPEG2000 : ✅")
    print(f"  LLM      : Groq (llama-3.3-70b + llama-3.1-8b)")
